...
- [ ] **Nothing reads like machine translation.** Legal Vietnamese has settled
      phrasing; if a sentence reads like it was translated word-by-word from
      English, flag it.
- [ ] **The `"AS IS"` term is left in English with a Vietnamese gloss**, in both
...
- [ ] **All four VI pages carry the banner** saying the English is canonical,
...
- [ ] **`Phiên bản 1.0-draft` shows at the bottom** of every page, both
      languages. It must match what the Legal Gate stores.
- [ ] **The EULA's two in-text links work and stay in your language** — from
      ...
      reverse.
- [ ] **The language switch round-trips** on a legal page and keeps you on the
      same document.
- [ ] **`← Về trang chủ`** goes to `/`, and its English counterpart to `/en/`.
- [ ] **The wide licence table on `/legal/third-party` scrolls inside its own
...
- [ ] **Legal Gate** (first run, or after "Đặt lại ứng dụng"): the four links at
...
- [ ] **⚙ → Chung → Pháp lý**: the three links open on-site.
- [ ] **⚙ → Giới thiệu**: "Ghi chú bên thứ ba" opens on-site. **"Mã nguồn" must
      still go to GitHub** — that is the GPL-3.0 §6 source offer and is not a
      site route.
- [ ] 🔴 **Switch the app to English (⚙ → Ngôn ngữ), then re-open those links.**
...
- [ ] **The landing footer**, on `/` and again on `/en/` — same check, four
      links each.

## Non-blocking

- [ ] **Lighthouse ≥ 95 on one legal page** — desktop, all four categories.
...
- [ ] Real Firefox: one legal page in each language.
- [ ] The prose is comfortable to read at a normal window width.
...
- [ ] `ticktune.net/app/` → ⚙ → Giới thiệu reads **0.10.0**. The line that
      proves the *tag* deployed rather than the branch push that preceded it.
- [ ] All eight `/legal/*` routes return 200 on the custom domain.
