...
- [x] **Tắt** — no graphic behind the countdown. This is the default, and a
      fresh profile must start here.
- [x] **Cột** — bars along the bottom, mirrored out from the centre, moving with
      the music. Bass on the inside, treble towards the edges.
- [x] **Sóng** — a single line across the middle that moves with the waveform.
- [x] **Vòng** — the signature look: short bars in a circle around the digits,
      not crossing them.
- [x] **Độ nhạy** — drag it up and the movement gets bigger; drag it down and it
      calms. At maximum nothing should look clipped or frozen.
- [x] **The countdown stays readable in every style.** If the graphic makes the
...
- [x] **Playing a local track, the dot pulses with the beat.** Red, and it
      visibly grows and glows on each kick.
- [x] **Set Hiệu ứng nhạc → Tắt. The dot still pulses.** This is the single most
...
- [x] **In YouTube mode the dot is red but steady** — no pulse. That is correct
...
- [x] **Set the countdown to 15 seconds and start it.** You should hear exactly
      **two** announcements: "Còn mười giây", then "Hết giờ". Nothing at the
      start, and nothing in between.
- [x] **Nothing about minutes.** A fifteen-second countdown must never say "Còn
      một phút" — it never had a minute. This exact bug existed during
      development and is what the line above is watching for.
- [x] **Set 11 minutes and start.** Within the first minute you should hear
      "Còn mười phút" — and nothing else until five minutes.
- [x] **The countdown does not read itself out every second.** If your screen
...
- [x] **The visualizer disappears entirely** in all three styles.
- [x] **The tally light stops pulsing** and sits steady.
- [x] **⚙ still shows your chosen style** — Vòng stays selected. Turn the OS
...
- [x] Backgrounds, the ⚙ panel and Focus mode all still behave.
- [x] With a YouTube video playing, **nothing is drawn over the player** — check
      with the visualizer on a local-mode track first, then switch to YouTube
      and confirm there is no canvas at all.

## Non-blocking

- [x] Switch to `EN` and reopen ⚙ → Visualizer reads as English.
- [x] A very quiet passage leaves the bars low rather than jumping to full.
- [x] **Real Firefox**, one pass of a style plus the tally pulse. CI's Firefox
      cannot produce audio at all (`13 §3`), so this is the only place the
      analyser is exercised there.
- [x] Leave a style running for a few minutes — it should not slow the page
...
- [x] `https://ticktune.net/app/` loads and ⚙ → Giới thiệu reads **0.8.0**.
- [x] One visualizer style moves with a local track, and the tally pulses.
