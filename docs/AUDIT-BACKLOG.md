...
Optional tightening (not part of the original finding, worth its own item): frame-src www.youtube.com is redundant given host: youtube-nocookie, and dropping it would make CSP a backstop for the invariant — but 06 §7 should then be edited in the same pass. ✅ **The s.ytimg.com half is REFUTED — measured 2026-07-22 against the live site.** This finding predicted that `script-src` omitting `https://s.ytimg.com` would break the player, because `www.youtube.com/iframe_api` was believed to inject `www-widgetapi.js` from there. The S1 harness at `/spike/s1-youtube`, running under the deployed CSP in a real browser, loaded it from **`https://www.youtube.com/s/player/<build>/www-widgetapi.vflset/www-widgetapi.js`** — the same origin `script-src` already allows. `securitypolicyviolation` fired zero times, the API was ready in 115 ms and the video reached `playing`. No CSP row and no privacy-policy disclosure are needed. Recorded rather than deleted, because "we checked and it was fine" is worth more to the next reader than silence.
